...
export async function handleCallback(
	...
	switch (action) {
		...
		case "x":
			await env.DB.prepare("DELETE FROM watches WHERE id = ?")
				.bind(watchId)
				.run();
		...
		case "m":
			if (arg === "tgt") {
				await env.DB.prepare(
					"UPDATE users SET pending_action = ? WHERE chat_id = ?",
				)
				...
				await tg.sendMessage(
					chatId,
					"What price should I alert you at? Send just the number.",
				);
			} else {
				await env.DB.prepare(
...
export async function sendList(
	...
	if (!list.length) {
		await tg.sendMessage(
			chatId,
			"No active watches. Send me a product link to start one.",
		);
		return;
	}
	...
	const lines = list.map((w) => {
		const price =
			w.last_price !== null ? formatPrice(w.last_price, w.currency) : "—";
		const target = w.target_price
			? ` · target ${formatPrice(w.target_price, w.currency)}`
			: "";
		const days = Math.max(
			0,
			Math.ceil((Date.parse(w.expires_at) - Date.now()) / 86_400_000),
		);
