...
export function findProduct(root: unknown): ProductInfo | null {
	...
	const walk = (node: unknown, depth: number): ProductInfo | null => {
		...
		if (isType(obj["@type"], "Product")) {
			...
			if (info.price !== undefined && info.price !== null && info.price !== "")
				return info;
		}
...
function pickOffer(offers: unknown, depth: number): OfferInfo | null {
	...
	if (Array.isArray(offers)) {
		...
		const parsed = offers
			...
			.filter(
				(o): o is OfferInfo => !!o && o.price !== undefined && o.price !== null,
			);
		if (!parsed.length) return null;
		return parsed.reduce((lo, cur) =>
			numeric(cur.price) < numeric(lo.price) ? cur : lo,
		);
	}
...
function isType(type: unknown, wanted: string): boolean {
	if (typeof type === "string")
		return type.toLowerCase().includes(wanted.toLowerCase());
